//	
//  SGNBPXLUUIDHandler.m
//  UUIDHandler
//
//	https://github.com/journeyapps/cordova-ios-uuid
//
...
@implementation SGNBPXLUUIDHandler
...
static CFMutableDictionaryRef CreateKeychainQueryDictionary(void)
{
	...
	if ([SGNBPXLUUIDHandler accessGroup])
	{
		CFDictionarySetValue(query, kSecAttrAccessGroup, (__BRIDGE CFTypeRef)[SGNBPXLUUIDHandler accessGroup]);
	}
#endif
	return query;
}
...
#pragma clang disgnostic ignored "-Wdeprecated-declarations"
...
+ (NSString *)storeUUID:(BOOL)itemExists
{
	...
	if (status != noErr)
	{
		NSLog(@"SGNBPXLUUIDHandler Keychain Save Error: %ld", status);
		uuid = nil;
	}
...
+ (NSString *)UUID
{
	...
	if (status != noErr) 
	{
		...
		{
			NSLog(@"SGNBPXLUUIDHandler Unhandled Keychain Error %ld", status);
			return nil;
		}
	}
...
+ (void)reset
{
	...
	if (status != noErr)
	{
		NSLog(@"SGNBPXLUUIDHandler Keychain Delete Error: %ld", status);
	}
	CFRelease(query);
}
