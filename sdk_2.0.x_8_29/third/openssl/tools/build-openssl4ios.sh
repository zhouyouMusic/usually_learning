...
SDK_VERSION=""12.2""
LIB_NAME="openssl-1.0.2s"
LIB_DEST_DIR="${pwd_path}/../output/ios/openssl-universal"
HEADER_DEST_DIR="include"
#rm -rf "${HEADER_DEST_DIR}" "${LIB_DEST_DIR}" "${LIB_NAME}"
 
if [ ! -e openssl-${LIB_NAME}.tar.gz ]; then
	echo "Downloading openssl-${LIB_NAME}.tar.gz"
	curl -O http://www.openssl.org/source/openssl-${LIB_NAME}.tar.gz
fi
...
configure_make()
{
   ...
   if [[ "${ARCH}" == "x86_64" ]]; then
       ...
       ./Configure BSD-generic32 --prefix="${PREFIX_DIR}"
#       ./Configure darwin-i386-cc --prefix="${PREFIX_DIR}"
   else
       ./Configure iphoneos-cross --prefix="${PREFIX_DIR}"
   fi
   export CFLAGS="-isysroot -fembed-bitcode ${CROSS_TOP}/SDKs/${CROSS_SDK}"
