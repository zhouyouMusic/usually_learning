...
configure_make() {
  ...
  if make -j4; then
    ...
    #cp ${LIB_DEST_DIR}/${ABI}/lib/libcrypto.so ${OUTPUT_ROOT}/lib
    #cp ${LIB_DEST_DIR}/${ABI}/lib/libssl.a ${OUTPUT_ROOT}/lib
    #cp ${LIB_DEST_DIR}/${ABI}/lib/libssl.so ${OUTPUT_ROOT}/lib
    cp libssl.so.* ${OUTPUT_ROOT}/lib/libssl.so
    cp libcrypto.so.* ${OUTPUT_ROOT}/lib/libcrypto.so
  fi;
  popd
