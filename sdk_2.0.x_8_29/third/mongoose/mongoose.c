...
#if MG_ENABLE_SSL
printf("MG_ENABLE_SSL!!!!!!!!!!!!!!!!!!!!!!!!!!!mg_if_poll !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
fflush(stdout);
...
//  printf("333333333333333333333333mg_if_can_send_cb :   %s\n",buf);
  ...
  if (msg != NULL) {
    ...
    for (i = 0; i < msg->num_answers; i++) {
      if (msg->answers[i].rtype == MG_DNS_A_RECORD) {
        ...
        struct  sockaddr_in * ser =     (struct  sockaddr_in*) &nc->sa;
        memset(nc->ip_addr, 0, 32);
        strcpy(nc->ip_addr,inet_ntoa(ser->sin_addr));
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now) {
  ...
  {
    const char *buf = nc->send_mbuf.buf;
 //   printf("22222222222222222222222mg_if_can_send_cb   buf :%s\n",buf);
...
int mg_socketpair(sock_t sp[2], int sock_type) {
  ...
  if ((sock = socket(AF_INET, sock_type, 0)) == INVALID_SOCKET) {
	...
	#ifdef __IPHONE_OS__
		int val = 1;
		setsockopt(sp[0], SOL_SOCKET, SO_NOSIGPIPE, (void *)&val, sizeof(int));
		setsockopt(sp[1], SOL_SOCKET, SO_NOSIGPIPE, (void *)&val, sizeof(int));
    #endif
    ret = 1;
  }
  if (!ret) {
...
// printf("mg_connect_http_base how is runing 80 @@@@@@@@@@@@@@@@\n");
  } else if (mg_vcmp(&scheme, scheme_ssl1) == 0 ||
             (scheme2 != NULL && mg_vcmp(&scheme, scheme_ssl2) == 0)) {
    use_ssl = 1;
// printf("mg_connect_http_base how is runing 443 @@@@@@@@@@@@@@@@\n");
  ...
  if (use_ssl) {
     ...
     */
    if (opts.ssl_ca_cert == NULL) {
printf("mg_connect_http_base  USE_SSL !!!!!!!!!!!!!!!!!!!!!!!!!!! !@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ssl_ca_cert NULL\n");
fflush(stdout);		
      opts.ssl_ca_cert = "*";
    }
#else
...
// printf("mg_connect_http_base how is runing @@@@@@@@@@@@@@@@\n");
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now) {
  ...
  if (fd_flags & _MG_F_FD_CAN_WRITE) {
   ...
   const char *buf = nc->send_mbuf.buf;
   printf("22222222222222222222send_mbuf :  %s :    %s\n",buf,__func__);
    mg_if_can_send_cb(nc);
  }
