...
void *speech_request(void *arg)
{
	...
	init_speech_recognition(&handleUSC,&recogParam,&MUTEX_UniSd);	
			...
			switch(recvCont.contType)
			{
				...
				case 2:
						if(flagStarted)
						{
							...
							{
								automatic_speech_recognition(handleUSC,recvedBuff+(i*640),640,uniSdOut,&MUTEX_UniSd);
								i++;
								recvedCount -= 640;
							}
				...
				case 3:
						...
						{
							if(restLength > 0)
								automatic_speech_recognition(handleUSC,restBuff,restLength,uniSdOut,&MUTEX_UniSd);
