...
static int set_hwparams(AlsaContext *context,snd_pcm_access_t access)
{
	...
	if (err < 0) {
		BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
	...
	if (err < 0) {
		BASE_ERROR_LOG(ErrLogHandle,"%s%i%s%s",
			"Channels count (",context->channels,") not available for playbacks: ",snd_strerror(err));
		return err;
	}
	/* set the stream rate */
	rrate = context->rate;
	...
	if (rrate != context->rate) {
		BASE_ERROR_LOG(ErrLogHandle,"%s%i%s%i%s",
			"Rate doesn't match (requested ",context->rate,"Hz, get ",err,"Hz)\n");
		return -EINVAL;
	}
	/* set the buffer time */
	err = snd_pcm_hw_params_set_buffer_time_near(context->handle, context->hwparams, &context->buffer_time, &dir);
	if (err < 0) {
		BASE_ERROR_LOG(ErrLogHandle,"%s%i%s%s%s",
			"Unable to set buffer time ",context->buffer_time," for playback: ",snd_strerror(err),"\n");
		return err;
	}
	...
	if (err < 0) {
		BASE_ERROR_LOG(ErrLogHandle,"%s%i%s%s%s",
			"Unable to set period time ",context->period_time," for playback: ",snd_strerror(err),"\n");
		return err;
	}
...
//		printf("Unable to get period size for playback: %s\n", snd_strerror(err));
		BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
			"Unable to get period size for playback: ",snd_strerror(err),"\n");
		return err;
	}
...
//		printf("Unable to set hw params for playback: %s\n", snd_strerror(err));
		BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
			"Unable to set hw params for playback: ",snd_strerror(err),"\n");
		return err;
	}
	return 0;
}
...
//			printf("Unable to set period event: %s\n", snd_strerror(err));
			BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
				"Unable to set period event: ",snd_strerror(err),"\n");
			return err;
		}
	}
...
extern AlsaHandle alsaInit(char *deviceName,short modle,unsigned int channels,unsigned int rate,unsigned int format) {
	...
	context->modle = modle;
	if (modle == 0) {
		context->buffer_time = 50000;// 500000;
		context->period_time = 1000;///10000;
	#if 0
		if(rate == 22050)
		{
			context->buffer_time = 5000;
		}else{
...
//			printf("Playback open error: %s\n", snd_strerror(err));
			BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
					"Playback open error:  ",snd_strerror(err),"\n");
			return -1;
		}
	} else {
...
//			printf("record open error: %s\n", snd_strerror(err));
			BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
					"record open error:   ",snd_strerror(err),"\n");
			return -1;
		}
	}
...
//		printf("Setting of hwparams failed: %s\n", snd_strerror(err));
		BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
					"Setting of hwparams failed:    ",snd_strerror(err),"\n");
		return -1;
	}
	if (modle == 0) {
...
//			printf("Setting of swparams failed: %s\n", snd_strerror(err));
			BASE_ERROR_LOG(ErrLogHandle,"%s%s%s",
					"Setting of swparams failed:     ",snd_strerror(err),"\n");
			return -1;
		}
	}
