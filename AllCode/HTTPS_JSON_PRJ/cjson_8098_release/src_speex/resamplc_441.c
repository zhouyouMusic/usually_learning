...
{
 	int16_t input[40960];
 	int16_t output[81920];
 	FILE * fdr = fopen("mj.pcm","rb");
 	FILE * fdw = fopen("hello.pcm","wb");
 	int sr=48000;//原始采样率
 	int target_sr=44100;//重采样后采样率
	int in_len,out_len;
	SpeexResamplerState *resampler = speex_resampler_init(2, sr, target_sr, 10, NULL);//初始化
 	speex_resampler_skip_zeros(resampler);
 	while (1)
 	{   
 		int	readed=fread(input,2,1152/2,fdr);
 		if (readed<=0)
 		{   
 			break;
 		}
 		in_len=readed; 
 		out_len=1152/2 * 4;//输出缓冲大小
