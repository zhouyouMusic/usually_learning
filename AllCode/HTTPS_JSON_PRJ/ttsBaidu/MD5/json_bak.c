...
char* subJsonStr(char *jsonStr)
{
	...
	char *jsonBufStr = (char *)calloc(50,QUES_BUF);
	if(NULL == jsonBufStr)
	{
...
int main()
{
	...
	char * jsonBuf = (char *)calloc(50,JSON_BUF);
	char * jsonBufKeep = (char *)calloc(50,JSON_BUF);
	...
	if(flag == 1)
	{
		memset(jsonBufDeal,0,30*JSON_BUF);
		jsonBufKeep = jsonBufDeal;
		while(*jsonBuf!='\0')		/***	除去歌词	***/
		{
			if((*(jsonBuf+1)=='l')&&(*(jsonBuf+2)=='y'))
			{
				if(!strncmp(jsonBuf+1,"lyrics\":\"",strlen("lyrics\":\"")))
				{
					while(*(jsonBuf+2)!='\0')
					{
						if((*jsonBuf=='\"')&&(*(jsonBuf+1)==','))
