...
FRAME_NODE * header = NULL;
//FRAME_NODE * temp = NULL;

void createNode(FRAME data)
{
	FRAME_NODE * temp = header;
	FRAME_NODE * node = (FRAME_NODE *)malloc(1*sizeof(FRAME_NODE));
	node -> next = NULL;
	node -> data = data;
//	strncpy(node->data.fram,data.fram,data.length);
...
{
	int i = 0,count = 0;
    header = (FRAME_NODE *)calloc(1,sizeof(FRAME_NODE));
	header -> next = NULL;
	fp1 = fopen("tmp.pcm","wb");
...
//    snd_pcm_writei (handle, OutputPtr, n);  
	...
	FRAME frame;
	frame.length = 4*n;
	frame.fram = (char*)malloc(4*n);
	memcpy(frame.fram,OutputPtr,4*n);
//	strncpy(frame.fram,OutputPtr,2*n);
    createNode(frame);
#endif 
	OutputPtr = Output;     
...
static int decode(unsigned char const *start, unsigned long length)
{
  ...
  mad_decoder_finish(&decoder);
	...
	int count = 0;
	FRAME_NODE* temp = header -> next;
	while(temp!=NULL)
	{
//		fwrite(temp->data.fram,temp->data.length,1,fp1);
		
		snd_pcm_writei(handle, temp->data.fram, temp->data.length/4);
		if(count==500)
		{
			snd_pcm_pause(handle,1);
			snd_pcm_drop(handle);
			ret = snd_pcm_readi(handle,temp->data.fram,temp->data.length/4);
			if(ret=!EBADFD)
			{
				printf("not running	\n");
			}
